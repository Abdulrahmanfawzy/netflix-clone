...
export default function MovieDetails() {
  ...
  let [loveChecked , setLoveChecked] = useState(false);
  let [movie_id,setMovie_id] = useState(movieId);
  ...
  useEffect(()=>{
    setLoveChecked(false);
    setWatchChecked(false);
  },[movieId])

  useEffect(()=>{
    getAllWatchList();
    getAllLoveList();
  },[watchChecked,loveChecked , movieId])
  ...
  async function getMovieById(id) {
    let { data } = await getMovieDetails(`movie/${id}`);
    setItems(data);
  }

  async function getAllCast() {
    let { data } = await getAllCastMovie(`movie/${movieId}`);
    setCast(data.cast);
  }

  async function getAllVideos() {
    let { data } = await getVideosOfMovie(`movie/${movieId}`);
    setVideo(data.results);
  }

  async function getAllPosters() {
    let arr = [];
    let { data } = await getPostersOfMovie(`movie/${movieId}`);
    if (data?.posters) {
      for (let i = 0; i < 10; i++) {
  ...
  async function getAllRecommendation() {
    let arr = [];
    let { data } = await getRecommendationOfMovie(`movie/${movieId}`);
    if (data) {
  ...
  function addReviewFun() {
    if (!val) {
      ...
      set(ref(db, `users/reviews/${movieId}/` + Date.now()), {
        username: username,
        createdAt: Date.now(),
        review: val,
        userid: userid,
  ...
  function getAllReviews() {
    ...
    let newArr = [];
    let secondArr = []
    get(child(dbRef, `users/reviews/${movieId}`))
      .then((snapshot) => {
        if (snapshot.exists()) {
          let data = snapshot.val();
  ...
  function removeReview(review_id , index){
    ...
    remove(ref(db, `users/reviews/${movieId}/`+review_id))
    .then(res=>{
  ...
  function watchlistFun(id,movieTitle, poster , vote , date){
    ...
    .then((res) => {
      toast.success(`${movieTitle} Movie has been added successfully`);
    })
    .catch((err) => {
      console.log(err);
    });
  ...
  function handleLoveIcon(id,movieTitle, poster , vote , date){
    setLoveChecked(true);
    set(ref(db, `users/userAuth/${userid}/lovelist/` + id), {
      userid: userid,
      movieId: id,
      movieTitle: movieTitle,
      poster: poster,
  ...
  useEffect(() => {
    ...
    getAllPosters(movieId);
    getAllRecommendation(movieId);
  }, [movieId]);

  return (
    <>
      <HelmetFun title={items?items.title : "Movie"}/>
      {items ? (
        <>
          <div
          ...
          <div className="basic_container allPage">
            <div className="row">
              ...
              <div className="col-md-7">
                <h2 className="title_movie">{items.title}</h2>
                ...
                <div className="d-flex gap-4 align-items-center">
                  <div onClick={()=>handleLoveIcon(items.id , items.title , items.poster_path , items.vote_average , items.release_date)} className="love" style={{ cursor: "pointer" }}>
                    {loveChecked? <i className="fa-solid fa-heart"></i>
                    :<i className="fa-regular fa-heart"></i>}
                    
                  </div>
                  <div>
                    <button
                      onClick={()=>watchlistFun(items.id , items.title , items.poster_path , items.vote_average , items.release_date)}
                ...
                {recommendations?.length > 0
                  ? recommendations.map((item, index) => (
                      ...
                      >
                        ...
                        <Link
                          onClick={() => handleMovieDisplay(item.id)}
                          to={`/recommenditem/${item.id}/moviedetails`}
